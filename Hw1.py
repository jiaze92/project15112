...
def pascalsTriangleValue(row, col):
    if row < 0 or col < 0 or col > row:
        ...
        pascals = roundHalfUp(math.factorial(row) /
...
def numberOfPoolBallRows(balls):
    return math.ceil((math.sqrt(8 * balls + 1) - 1) / 2)


def colorBlender(rgb1, rgb2, midpoints, n):
    r1 = (rgb1 // 10 ** 6)
    g1 = (rgb1 - r1 * 10 ** 6) // 10 ** 3
    b1 = (rgb1 - r1 * 10 ** 6 - g1 * 10 ** 3)
    r2 = (rgb2 // 10 ** 6)
    g2 = (rgb2 - r2 * 10 ** 6) // 10 ** 3
    b2 = (rgb2 - r2 * 10 ** 6 - g2 * 10 ** 3)
    rstep = (r1 - r2) / (midpoints + 1)
    rn = roundHalfUp(r1 - rstep * n)
    gstep = (g1 - g2) / (midpoints + 1)
    gn = roundHalfUp(g1 - gstep * n)
    bstep = (b1 - b2) / (midpoints + 1)
    bn = roundHalfUp(b1 - bstep * n)
    rgbn = rn * 10 ** 6 + gn * 10 ** 3 + bn
    if n >= midpoints + 2 or n < 0:
        return None
    else:
        return rgbn
